...
class ShopLogoLookupDialog(QDialog):
    def __init__(self, canonical_shop_name, candidates, sender_domain='', parent=None):
        ...
        self.setWindowTitle('Shoplogo aus Web-Suche waehlen')
    ...
    def _build_ui(self):
        ...
        context_text = self.canonical_shop_name or self.sender_domain or '-'
        lbl = QLabel(
            f"Shop: <b>{context_text}</b><br>"
            "Waehle genau ein Logo aus und klicke auf 'Uebernehmen'."
        )
    ...
    def _populate_table(self):
        if not self.candidates:
            self.lbl_empty.setText('Es wurden keine passenden Logo-Treffer gefunden.')
            self.table.setRowCount(0)
            self.btn_apply.setEnabled(False)
            return

        self.lbl_empty.setText('Bis zu 3 passende Logo-Treffer wurden gefunden.')
        ...
        for row_index, candidate in enumerate(self.candidates):
            ...
            if pixmap is not None:
                preview.setPixmap(
                    pixmap.scaled(84, 56, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                )
    ...
    @staticmethod
    def choose(canonical_shop_name, candidates, sender_domain='', parent=None):
        dialog = ShopLogoLookupDialog(canonical_shop_name, candidates, sender_domain=sender_domain, parent=parent)
