...
3. Seriennummer scannen (speichert S/N, verknüpft Artikel mit Paket, ändert Status).
"""
...
class PackstationApp(QWidget):
    def __init__(self, settings_manager):
        ...
        # State Machine für den Scan-Modus
        ...
        self.scanned_items_in_box = [] # Für die UI-Tabelle (Logs)
    ...
    def _build_ui(self):
        ...
        btn_reset = QPushButton("🔄 Vorgang abbrechen / Neues Paket")
        ...
        try:
            ...
            if res:
                ...
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s)
                """, (tracking_code, now, shipment_db_value(ShipmentStatus.NOT_DISPATCHED)))
    ...
    def _process_ean(self, ean_code):
        ...
        try:
            ...
            # WICHTIG: Wir prüfen, ob der Artikel IN_STOCK ist und noch KEINEM ausgangs_paket zugeordnet ist.
            # (Limit 1, wir ignorieren vorerst Menge > 1 Logik pro Eintrag, falls wir Seriennummern pro Eintrag fordern).
            # Um es sicher zu machen: Die Tabelle waren_positionen sollte für Seriennummern ideally Stückelung 1 haben,
            # aber falls menge > 1, ziehen wir 1 ab oder binden die Seriennummer an die Posiotion.
            # Fürs erste binden wir die Seriennummer an die Position. Wenn menge > 1, wird es komplex.
            
            query = """
                SELECT id, produkt_name, ean 
                FROM waren_positionen 
                WHERE ean = %s AND status = %s AND (ausgangs_paket_id IS NULL OR ausgangs_paket_id = 0)
                LIMIT 1
            """
            cursor.execute(query, (ean_code, InventoryStatus.IN_STOCK.value))
            res = cursor.fetchone()
    ...
    def _process_serial(self, serial_code):
        """Speichert die Seriennummer beim Artikel, verknüpft das Paket und ändert den Status"""
        try:
            conn = self.db._get_connection()
            cursor = conn.cursor()
            ...
            cursor.execute("""
                ...
                SET seriennummern = %s, ausgangs_paket_id = %s, status = %s
                WHERE id = %s
            """, (serial_code, self.current_paket_id, InventoryStatus.SHIPPED.value, self.current_item_id))
            ...
            # Zurück zu EAN scannen (für den nächsten Artikel ins gleiche Paket)
            self.scan_state = "WAITING_FOR_EAN"
            self.lbl_instruction.setText("Gespeichert! ✔\nScanne NÄCHSTE EAN oder wähle 'Neues Paket'")
            ...
            log_exception(__name__, e)
            QMessageBox.critical(self, "DB Fehler", f"Fehler bei Seriennummer-Verknüpfung:\n{e}")
