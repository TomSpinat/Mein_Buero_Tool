...
from PyQt6.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QApplication
...
class DashboardWindow(QMainWindow):
    ...
    def __init__(self, settings_manager=None):
        ...
        self._ensure_reasonable_window_size(1024, 600)
    ...
    def resizeEvent(self, event):
        ...
        if screen is not None:
            ...
            max_width = max(960, int(available.width() * 0.9))
            max_height = max(640, int(available.height() * 0.9))
    ...
    def _ensure_reasonable_window_size(self, preferred_width, preferred_height):
        screen = self.windowHandle().screen() if self.windowHandle() else QApplication.primaryScreen()
        if screen is None:
            return
        available = screen.availableGeometry()
        max_width = max(960, int(available.width() * 0.9))
        max_height = max(640, int(available.height() * 0.9))
        target_width = min(max_width, max(int(preferred_width or 0), 960))
        target_height = min(max_height, max(int(preferred_height or 0), 640))
        current_width = max(0, int(self.width() or 0))
        current_height = max(0, int(self.height() or 0))
        if current_width > max_width or current_height > max_height or current_width < 320 or current_height < 240:
    ...
    def open_scanner(self):
        ...
        self._ensure_reasonable_window_size(1480, 920)
    ...
    def return_to_main(self):
        """Wechselt zurueck zum Hauptmenue (Dashboard)"""
