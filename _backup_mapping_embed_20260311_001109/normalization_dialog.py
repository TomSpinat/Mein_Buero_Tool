...
class NormalizationDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        info_lbl = QLabel(f"Neuer Begriff für <b>{cat_name}</b> erkannt:\n"
        ...
        # Primäre Option: Als neuen Standard speichern
        lbl_new = QLabel("<b>Primäre Auswahl:</b> Diesen Begriff als neuen Standard speichern")
        ...
        # Sekundäre Option: Bestehenden Standard wählen
        ...
        standards.sort()
        self.combo_standards.addItem("--- Nicht ändern, sondern obigen Text beibehalten ---")
        ...
        # Wenn der User in das neue Feld tippt, setzen wir die Combo zurück
        self.entry_new.textChanged.connect(lambda: self.combo_standards.setCurrentIndex(0))
        # Wenn der User die Combo ändert, leeren wir das neue Feld
    ...
    def save_selection(self):
        ...
        if not final_standard:
            QMessageBox.warning(self, "Eingabe fehlgeschlagen", "Bitte wähle einen Standard oder trage einen neuen ein.")
            return
...
def normalize_value(category, raw_value, parent_widget=None):
    ...
    Ist er im mapping.json, wird der Standard zurückgegeben.
    ...
    :param parent_widget: Optionales QWidget, über dem der Dialog zentriert wird
