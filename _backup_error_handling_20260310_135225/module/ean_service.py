...
class EanService:
    ...
    def lookup_candidates_by_name(self, produkt_name, varianten_info="", limit=25, allow_api_fallback=True):
        ...
        if local_candidates:
            return {
                ...
                "candidates": local_candidates[:max_limit],
                "used_api": False,
                "local_hit": True,
            }
        ...
        api_candidates = self._search_upcitemdb_candidates(normalized, max_limit)
        return {
            "normalized": normalized.to_dict(),
            "candidates": api_candidates[:max_limit],
            "used_api": True,
            "local_hit": False,
        }
    ...
    def _fetch_upcitemdb_items(self, query_text):
        ...
        try:
            ...
            log_exception(f"{__name__}._fetch_upcitemdb_items", e, extra={"query": query})
            return []
    ...
    def _search_upcitemdb_candidates(self, normalized, limit=25):
        ...
        for query_info in query_variants:
            ...
            raw_items = self._fetch_upcitemdb_items(query_text)
            log_message(
                f"{__name__}._search_upcitemdb_candidates",
                "query completed",
                extra={
    ...
    def remember_mapping(self, produkt_name, ean, varianten_info="", bild_url="", quelle="manual", confidence=1.0):
        ...
        return self.db.upsert_local_ean_mapping(
            produkt_name=produkt_name,
            ean=ean,
            varianten_info=varianten_info,
            bild_url=bild_url,
