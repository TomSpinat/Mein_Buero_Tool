...
from PyQt6.QtWidgets import (
  ...
  QComboBox, QLineEdit, QSizePolicy, QFormLayout
)
...
class MailScraperApp(QWidget):
  ...
  def _on_scan_finished(self, extracted_data_list, highest_uid=-1, account_idx=-1):
    ...
    try:
      ...
      log_exception(__name__, fatal_err)
      tb = traceback.format_exc()
      self._log(f"FATAL _on_scan_finished: {fatal_err}")
      try:
        with open(resource_path("mail_scraper_crash.log"), "a", encoding="utf-8") as f:
          f.write("\n=== _on_scan_finished crash ===\n")
          f.write(tb)
          f.write("\n")
      except Exception as e:
        log_exception(__name__, e)
      QMessageBox.critical(self, "Kritischer Fehler", f"Absturz im Uebernehmen-Flow:\n{fatal_err}\n\nDetails wurden in mail_scraper_crash.log geschrieben.")
...
class GeminiEmailThread(QThread):
  """Verarbeitet eine Liste von (screenshot_path, raw_email_dict) mit Gemini.
  LÃ¤uft im Hintergrund damit die UI nicht einfriert."""
  ...
  def run(self):
    ...
    for i, (img_path, raw) in enumerate(self.screenshot_paths):
      ...
      self._log(f" ðŸ¤– [{i+1}/{len(self.screenshot_paths)}] KI analysiert: {str(subject)[:40]}...")

      max_retries = 3
      retry_count = 0

      # Datum-Hinweis als kurzen Begleittext mitschicken
      date_hint = f"Empfangsdatum der E-Mail: {email_date_raw}. Nutze dies als Kaufdatum-Fallback."

      while retry_count < max_retries:
        try:
          time.sleep(2)
          if img_path and os.path.exists(img_path):
            # Screenshot-Modus: Bild + kurzer Text-Hint
            result = process_receipt_with_gemini(
              self.api_key,
              image_path=img_path,
              custom_text=date_hint
            )
          else:
            # Fallback: Reiner Text (z.B. wenn Rendering fehlschlug)
            fallback_text = date_hint + "\n\n" + str(body_html if body_html else body_text)[:10000]
            result = process_receipt_with_gemini(
              self.api_key,
              image_path=None,
              custom_text=fallback_text
            )
          ...
          if result and isinstance(result, dict):
            ...
            if has_relevant_data:
              self.log_signal.emit(f" âœ… Erfolgreich extrahiert! (Tokens: {tokens})")
          ...
          log_exception(__name__, e)
          err = str(e)
          if "429" in err or "quota" in err.lower() or "Free-Tier" in err:
            retry_count += 1
            self.log_signal.emit(f"  Gemini Limit! Pausiere 60s... (Versuch {retry_count}/{max_retries})")
            for t in range(60, 0, -1):
              if t % 15 == 0 or t <= 5:
                self.log_signal.emit(f" ... {t}s verbleiben.")
              time.sleep(1)
            self.log_signal.emit(" ðŸ”„ Neuer Versuch...")
          else:
            self.log_signal.emit(f"  KI Fehler: {err[:80]}")
            self.log_signal.emit(traceback.format_exc()[-200:])
            break

      # Screenshot-Tempfile aufrÃ¤umen
      if img_path:
        try:
          os.remove(img_path)
        except:
...
class MailScraperThread(QThread):
  """Holt E-Mails via IMAP (READ-ONLY) und gibt rohe HTML-Bodies zurÃ¼ck.
  Gemini-Verarbeitung Ã¼bernimmt GeminiEmailThread."""
  log_signal   = pyqtSignal(str)
    ...
    try:
      ...
      for idx, mail_id in enumerate(recent_ids):
        ...
        for response_part in msg_data:
          ...
          self.log_signal.emit(f"-> PrÃ¼fe: {str(subject)[:50]}")
            ...
            for part in msg.walk():
              ...
              try:
                ...
                if ct == "text/plain":
                  ...
                  body_html += decoded
                elif content_id and ct.startswith("image/"):
          ...
          self.log_signal.emit(f" ðŸ“¬ Mail eingesammelt fÃ¼r Screenshot-Rendering")
          raw_emails.append({
            "subject": str(subject),
            "sender": str(sender),
...
class ClickableLabel(QLabel):
  """Ein QLabel das bei Klick ein vergrÃ¶ÃŸertes Bild zeigt."""
...
class ScraperReviewWizardDialog(QDialog):
  ...
  def _build_ui(self):
    ...
    lbl_left = QLabel("E-Mail Vorschau")
    ...
    left_box.addWidget(self.lbl_preview_notice)
    ...
    left_box.addLayout(preview_action_row)
    ...
    left_box.addWidget(self.preview_web, 1)
    ...
    left_box.addWidget(self.btn_open_large_preview)
  ...
  def _update_mapping_state_ui(self):
    ...
    if done:
      ...
      self.btn_run_mapping.setText("Mapping erneut oeffnen")
    else:
  ...
  def _load_current_mail(self):
    ...
    for key, _label in self.FIELD_DEFS:
      self.inputs[key].setText(self._safe_text(item.get(key, "")))

    self._render_current_preview()
    self._populate_items_table(item.get("waren", []))
  ...
  def _auto_prompt_mapping_for_index(self, idx):
    ...
    reply = QMessageBox.question(
      self,
      "Mapping fuer diese Mail",
      "Soll jetzt das Mapping (Shop/Zahlungsart/Amazon-Land) gestartet werden?\n\n"
      "Empfehlung: Yes, damit du alle Dialoge im Mail-Kontext erledigst.",
      QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
      QMessageBox.StandardButton.Yes,
    )
    if reply == QMessageBox.StandardButton.Yes:
      self._run_mapping_for_current_mail(show_feedback=False)
  ...
  def _on_run_mapping_clicked(self):
    self._run_mapping_for_current_mail(show_feedback=True)

  def _run_mapping_for_current_mail(self, show_feedback=True):
    try:
      payload = self._collect_current_payload()
      payload = EinkaufPipeline.normalize_einkauf_result(self, payload)
      ...
      self._populate_items_table(payload.get("waren", []))

      self._mapping_done_by_index[self.current_index] = True
      self._update_mapping_state_ui()

      if show_feedback:
        QMessageBox.information(self, "Mapping", "Mapping wurde uebernommen.")
    except Exception as e:
      log_exception(__name__, e)
    ...
    try:
      if not self._mapping_done_by_index.get(self.current_index, False):
        QMessageBox.information(
          self,
          "Mapping offen",
          "Bitte zuerst 'Mapping jetzt starten' ausfuehren.\n"
          "So sind Mapping-Dialoge im Bearbeitungsschritt und nicht beim Speichern."
        )
        return
