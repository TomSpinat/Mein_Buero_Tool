...
Wenn du die .exe spÃ¤ter startest, wird dieser Code hier als Erstes ausgefÃ¼hrt.

Wir zeigen hier zuerst ein komplett schwarzes, transparentes Pixel-Logo (Intro-Bild),
spielen Sound ab, und blenden dann nach der Sound-LÃ¤nge weich zum Hauptdashboard Ã¼ber.
"""
...
from module.dashboard import DashboardWindow  # Wir bauen das Dashboard im nÃ¤chsten Schritt!
...
class FadeOverlay(QWidget):
    """
    Diese kleine Hilfsklasse wird ein schwarzes Viereck zeichen, das Ã¼ber dem
    ...
    # Ein sogenanntes "Property" in PyQt, das wir animieren kÃ¶nnen
    @pyqtProperty(float)
    def opacity(self):
    ...
    def paintEvent(self, event):
        """Zeichne ein schwarzes Rechteck in der GrÃ¶ÃŸe des Fensters mit der aktuellen Durchsichtigkeit"""
        if self._opacity > 0:
...
class IntroSplashScreen(QWidget):
    ...
    def __init__(self, settings_manager):
        ...
        # Lade das transparente Intro-Bild Ã¼ber unser resource_path (Wichtig fÃ¼r PyInstaller!)
        pixmap = QPixmap(resource_path("assets/intro_bild.png"))
        self.lbl_image.setPixmap(pixmap)
        
        # Passe die FenstergrÃ¶ÃŸe exakt an das Bild an (PixelauflÃ¶sung)
        ...
        # Wenn die Audio fertig asynchron geladen wurde, hÃ¶ren wir darauf:
        self.sound.statusChanged.connect(self._on_sound_status_changed)
        
        # FÃ¼r den weichen Fade-Effekt am Ende
        self.fade_overlay = FadeOverlay(self)
        self.fade_overlay.resize(self.size())
        
        # Die Animation, die spÃ¤ter unsere opacity(Durchsichtigkeit) von 0 auf 1 Ã¤ndert
        ...
        # Wenn die Animation zu Ende ist, schlieÃŸen wir das Fenster endgÃ¼ltig
    ...
    def _on_sound_status_changed(self):
        """Diese Funktion lÃ¶st aus, wenn die Sound-Datei fertig geladen ist."""
        if self.sound.status() == QSoundEffect.Status.Ready:
            ...
            # Alternativ nutzt man QMediaPlayer, aber QSoundEffect reicht fÃ¼r kurze Intros.
            # Wir nehmen fÃ¼r das Intro pauschal 3,5 Sekunden an (3500ms).
            ...
            # kurz bevor der Ton ganz aufhÃ¶rt.
            # (Wir subtrahieren 1000ms von der LÃ¤nge)
            fade_start_ms = max(0, sound_length_ms - 1000)
            
            # QTimer fÃ¼hrt eine Funktion nach X Millisekunden aus
    ...
    def launch_dashboard(self):
        ...
        # 1. ZUERST das Dashboard Ã¶ffnen, damit immer ein Fenster fÃ¼r Qt aktiv ist 
        ...
        # 2. DANN das Intro-Fenster schlieÃŸen
        self.close()
...
def main():
    ...
    try:
        import ctypes
        # FÃ¼r Windows: Die Taskleiste dazu zwingen, unser eigenes Icon anzuzeigen (verhindert gruppieren mit python.exe)
        myappid = 'just.business.tools.main.1' 
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(myappid)
        
        app = QApplication(sys.argv)
        
        # Das Window Icon fÃ¼r alle Fenster der App setzen
        ...
        # --- NEU: Einstellungen einlesen & Datenbank Schema prÃ¼fen/erstellen ---
        ...
        # Die exec-Schleife hÃ¤lt das Programm am Leben, 
        # andernfalls wÃ¼rde das Skript hier einfach beendet sein.
        sys.exit(app.exec())
    except Exception as e:
