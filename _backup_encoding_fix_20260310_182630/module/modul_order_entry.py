...
class GeminiWorker(QThread):
    ...
    def __init__(self, api_key, image_path, custom_text, scan_mode):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            log_classified_error(
                ...
                exc=e,
                extra={"scan_mode": self.scan_mode, "image_path": str(self.image_path or "")},
            )
...
class ImageDropBox(QFrame):
    """
    Drag & Drop Box und Ctrl+V Ãœberwachung.
    """
...
class OrderEntryApp(QWidget):
    """
    Das Modul fÃ¼r die Bestellerfassung per Bild/Clipboard (Rechnungsscanner).
    """
    ...
    def _build_left_side(self):
        ...
        self.radio_einkauf = QRadioButton("ðŸ›’ Einkauf (Rechnung)")
        self.radio_einkauf.setChecked(True)
        
        self.radio_verkauf = QRadioButton("ðŸ·ï¸ Verkauf (Discord-Ticket)")
    ...
    def _on_mode_changed(self):
        ...
        if self.radio_einkauf.isChecked():
            ...
            self.btn_save_db.setText("ðŸ’¾ Bestellung speichern")
        else:
            self.scan_mode = "verkauf"
            self.lbl_form.setText("<h3>2. Ticketdaten (Verkauf)</h3>")
            self.btn_save_db.setText("ðŸš€ Discord-Ticket speichern / ticket folgt")
    ...
    def _build_right_side(self):
        ...
        # Tabelle fÃ¼r Warenpositionen (Einkauf oder Verkauf)
        ...
        self.table_waren.horizontalHeader().setStretchLastSection(True) # Die letzte Spalte fÃ¼llt den Rest auf
        self.table_waren.verticalHeader().setDefaultSectionSize(45) # GrÃ¶ÃŸer wegen dem QLineEdit Padding
        ...
        self.btn_reset = QPushButton("ðŸ—‘ï¸ Formular leeren")
        self.btn_reset.clicked.connect(self._reset_form)
        button_layout.addWidget(self.btn_reset)

        self.btn_save_db = QPushButton("ðŸ’¾ Bestellung speichern")
    ...
    def _build_dynamic_form(self):
        ...
        if self.scan_mode == "einkauf":
            ...
            self.table_waren.setHorizontalHeaderLabels(["Produkt", "Variante", "EAN", "Menge", "StÃ¼ckpreis"])
        else:
            fields = [
                ("ticket_name", "Ticket-Name:"),
                ("kaeufer", "KÃ¤ufer/Nutzername:"),
    ...
    def _start_scan(self):
        ...
        self.scan_temp_file_path = temp_img_path

        self.btn_scan.setText("ðŸ”„ Analyse lÃ¤uft...")
        self.btn_scan.setEnabled(False)
        ...
        self.gemini_worker = GeminiWorker(api_key, temp_img_path, custom_text, self.scan_mode)
        self.gemini_worker.temp_img_path = temp_img_path
    ...
    def _fill_ui(self):
        """FÃ¼llt das GUI aus dem current_gemini_data Dictionary."""
    ...
    def _save_to_database(self):
        # UI Ã„nderungen zurÃ¼ck in das gemini dict spielen
        for db_key, line_edit in self.inputs.items():
        ...
        if self.scan_mode == "einkauf":
            ...
            try:
                ...
                msg_parts = []
                if matched_items:
                    msg_parts.append(f"{len(matched_items)} StÃ¼ck werden sofort mit vorhandenen Bestellungen verknÃ¼pft:\n")
                ...
                if pending_units:
                    if msg_parts:
                        msg_parts.append("")
                    msg_parts.append(f"{len(pending_units)} StÃ¼ck bleiben als 'ticket folgt' offen und werden spÃ¤ter automatisch nachverknÃ¼pft:\n")
                ...
                if not msg_parts:
                    msg_parts.append("FÃ¼r dieses Ticket wurden aktuell keine verwertbaren Positionen erkannt.")
                ...
                if result.get("pending_count", 0) > 0:
                    CustomMsgBox.information(
                        self,
                        "Gespeichert",
                        f"Ticket gespeichert. {result.get('matched_count', 0)} StÃ¼ck direkt verknÃ¼pft, {result.get('pending_count', 0)} StÃ¼ck warten als 'ticket folgt'."
                    )
                else:
                    CustomMsgBox.information(
                        self,
                        "Erfolg",
                        f"Ticket gespeichert und {result.get('matched_count', 0)} StÃ¼ck direkt verknÃ¼pft."
                    )
                self._reset_form()
    ...
    def _on_db_item_changed(self, item):
        """Wird ausgelÃ¶st, sobald eine Zelle hÃ¤ndisch bearbeitet wurde."""
        if self._loading_db:
            return
