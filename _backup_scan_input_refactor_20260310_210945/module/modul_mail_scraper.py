...
class GeminiEmailThread(QThread):
  ...
  Läuft im Hintergrund damit die UI nicht einfriert."""
        ...
        try:
          ...
          if result and isinstance(result, dict):
            ...
            if has_relevant_data:
              self.log_signal.emit(f" ✅ Erfolgreich extrahiert! (Tokens: {tokens})")
...
class MailScraperThread(QThread):
  """Holt E-Mails via IMAP (READ-ONLY) und gibt rohe HTML-Bodies zurück.
  Gemini-Verarbeitung übernimmt GeminiEmailThread."""
  log_signal   = pyqtSignal(str)
    ...
    try:
      ...
      for idx, mail_id in enumerate(recent_ids):
        ...
        for response_part in msg_data:
          ...
          self.log_signal.emit(f"-> Prüfe: {str(subject)[:50]}")
          ...
          self.log_signal.emit(f" 📬 Mail eingesammelt für Screenshot-Rendering")
          raw_emails.append({
            "subject": str(subject),
            "sender": str(sender),
...
class ClickableLabel(QLabel):
  """Ein QLabel das bei Klick ein vergrößertes Bild zeigt."""
