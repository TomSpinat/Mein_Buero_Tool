...
class WareneingangApp(QWidget):
    ...
    def _load_pending_orders(self):
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                self.combo_orders.addItem(label, row['id'])
    ...
    def _finish_inbound(self):
        ...
        try:
            ...
            cursor.execute("UPDATE einkauf_bestellungen SET sendungsstatus = 'Geliefert' WHERE id = %s", (self.current_order_id,))
            
            # 2. Positionen als auf Lager markieren
            cursor.execute("UPDATE waren_positionen SET status = 'IN_STOCK' WHERE einkauf_id = %s", (self.current_order_id,))
